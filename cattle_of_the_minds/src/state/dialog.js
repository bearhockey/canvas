var DIALOG = (function () {
  ...
  // ----------------
  dg.OpenDialog = function(strImage="", iImageWidth=0, iImageHeight=0, strBgColor=DEFAULT_BG_COLOR, arrButtons=[], bUpdate=true)
  {
    ...
    if (dg.arrButtons != null && dg.arrButtons.length > 0)
    {
      ...
      for (idx = 0; idx < iButtonsLength; ++idx)
      {
        dg.arrButtons[idx].SetX(iXPosition);
        iXPosition += dg.arrButtons[idx].GetWidth() + BUTTONS_SPACING;
      } // end of for loop
    }
  ...
  dg.Draw = function(ctx)
  {
    ...
    ctx.fillRect(0, 0, GetCanvasWidth(), GetCanvasWidth());
