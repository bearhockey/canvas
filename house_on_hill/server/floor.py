...
class Floor:
    def __init__(self, idx=-1, arrRooms=[]):
        self.idx = idx
        self.arrAllRooms = arrRooms # reference to list of all rooms to generate tiles from
        self.arrTiles = []
        firstTile = self.add_tile(0, 0, True)
        firstTile.arrDoors = [1,1,1,1]

    # wrapper function so the idx of the tile is accurate
    def add_tile(self, x, y, bNoRando=False):
        newTile = Tile(len(self.arrTiles), x, y)
        if not bNoRando:
            arrValidRooms = list(filter(lambda x: Room.filter_category(x, self.idx), self.arrAllRooms))
            if len(arrValidRooms) > 0:
                cRoom = random.choice(arrValidRooms)
                self.arrAllRooms.remove(cRoom)
                newTile.set_room_data(cRoom)
